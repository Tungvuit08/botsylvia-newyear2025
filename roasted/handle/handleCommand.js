  ...
  return `${n.toFixed(n < 10 && l > 0 ? 1 : 0)} ${units[l]}`;
   ...
   const prefixRegex = new RegExp(`^(<@!?${senderID}>|${escapeRegex((threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : PREFIX )})\\s*`);
    ...
    const adminbot = require('./../../config.json');
      ...
      if(typeof body === 'string' && body.startsWith(PREFIX) && !ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {return api.shareContact(`[ ADMIN ONLY ]\n────────────────────\n👤 Người dùng: ${name}\n⚠️ Chỉ admin bot mới có thể sử dụng bot\n👤 Liên hệ với admin để được hỗ trợ\n────────────────────\n⏳ Uptime: ${H+$+M+$+S}\n⏰ Time: ${Tm}`,uid, threadID, messageID);
    }
      if(typeof body === 'string' && body.startsWith(PREFIX) && !ADMINBOT.includes(senderID) && adminbot.adminPaseOnly == true) {return api.shareContact(`[ ADMIN PASE ONLY ]\n────────────────────\n👤 Người dùng: ${name}\n⚠️ Chỉ admin bot mới được sử dụng bot trong chat riêng\n👤 Liên hệ với admin để được hỗ trợ\n────────────────────\n⏳ Uptime: ${H+$+M+$+S}\n⏰ Time: ${Tm}`,uid, threadID, messageID);
   }
     if(typeof body === 'string' && body.startsWith(PREFIX) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) {return api.shareContact(`[ NDH ONLY ]\n────────────────────\n👤 Người dùng: ${name}\n⚠️ Chỉ người hỗ trợ bot mới có thể sử dụng bot\n👤 Liên hệ với admin để được hỗ trợ\n────────────────────\n⏳ Uptime: ${H+$+M+$+S}\n⏰ Time: ${Tm}`,uid, threadID, messageID);
   }
    ...
    if (latestFile) {
      ...
      if (checker.bestMatch.rating >= 0.5) {
        ...
        api.sendMessage({
          ...
          attachment: global.gojodev.splice(0, 1) // Thêm tệp đính kèm từ global
