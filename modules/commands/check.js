...
if ((!check && Object.keys(mentions).length == 0) || (!check && Object.keys(mentions).length == 1) || (!check && event.type == 'message_reply')) {
    ...
    body += `🍓 ${nameThread} 🍓
        ...
        ───────────────
        💬 Tin Nhắn Ngày: ${userTotalDay}
        🥇 Hạng Trong Ngày: ${userRankDay + 1}
        ───────────────
        💬 Tin Nhắn Tuần: ${userTotalWeek}
        🥈 Hạng Trong Tuần: ${userRankWeek + 1}
        ───────────────
        💬 Tổng: ${userTotal}
        🏆 Hạng Tổng: ${userRank + 1}

        📌 Thả cảm xúc "❤️" tin nhắn này để xem tổng tin nhắn của toàn bộ thành viên.
        `.replace(/^ +/gm, '');
