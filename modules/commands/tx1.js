...
const dice_images = [
"https://i.imgur.com/1OUAq3n.jpeg",
"https://i.imgur.com/sIJmedj.jpeg",
"https://i.imgur.com/jEqVaC4.jpeg",
"https://i.imgur.com/nZQwF0W.jpeg",
"https://i.imgur.com/TzxwXop.jpeg",
"https://i.imgur.com/vrR7LLA.jpeg"
];
...
module.exports.run = async function ({
  ...
  if (args.length == 0) {
    var abcd = `🎲 GAME LẮC TÀI XỈU 🎲\n────────────────\n${global.config.PREFIX}${this.config.name} cr → Tạo bàn tài xỉu\n${global.config.PREFIX}${this.config.name} out → Rời khỏi bàn tài xỉu\n${global.config.PREFIX}${this.config.name} xổ → Bắt đầu lắc xúc xắc\n${global.config.PREFIX}${this.config.name} info → Xem thông tin bàn\n${global.config.PREFIX}${this.config.name} end → Kết thúc bàn tài xỉu`
    return api.sendMessage({body: abcd, attachment: (await require('axios').get(`https://i.imgur.com/3SYW3nq.jpeg`, {
        responseType: 'stream'
    })).data
}, threadID, messageID);
  }
