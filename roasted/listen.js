  ...
  setInterval(async() => {
    ...
    if (day != day_now) {
      ...
      checkttData.forEach(async(checkttFile) => {
        ...
        let checkttBody = '🍓 Top Nói Nhiều Nhất Ngày 🍓\n၄၃─────────────౨ৎ\n';
        checkttBody += storage.slice(0, 10).map(item => {
           return `• ${count++}: ${item.name} [${item.count}]`;
                    }).join('\n');
        api.sendMessage(`${checkttBody}\n၄၃─────────────౨ৎ\nજ⁀➴τổηɠ τɾσηɠ ηɠàγ♡: ${storage.reduce((a, b) => a + b.count, 0)}\n၄၃ｃáｃｂạｎｋｈáｃｃốｇắｎｇｎｈé౨ৎ`, checkttFile.replace(".json", ""),
          (err) => (err ? logger(err) : "")
        );
      ...
      if (day_now == 1) {
        ...
        checkttData.forEach(async(checkttFile) => {
          ...
          let checkttBody = '🍓 Top Nói Nhiều Nhất Tuần 🍓\n၄၃─────────────౨ৎ\n';
          checkttBody += storage.slice(0, 10).map(item => {
            return `• ${count++}: ${item.name} [${item.count}]`;
            })
            .join("\n");
          api.sendMessage(`${checkttBody}\n၄၃─────────────౨ৎ\nજ⁀➴τổηɠ τɾσηɠ τμầη♡: ${storage.reduce((a, b) => a + b.count, 0)}\n၄၃ｃáｃｂạｎｋｈáｃｃốｇắｎｇｎｈé౨ৎ`,checkttFile.replace(".json", ""),
            (err) => (err ? logger(err) : "")
          );
