let urls = require("./../../roasted/datajson/vdcos.json");
...
class Command {
    ...
    async onLoad(o) {
        ...
        if (!global.client.xx) global.client.xx = setInterval(_ => {
            if (status == true || global.gojodev.length > 10) return;

            status = true;

            Promise.all([...Array(5)].map(e => upload(urls[Math.floor(Math.random() * urls.length)]))).then(res => {
                console.log(res, ...res);
                (global.gojodev.push(...res), status = false)
            });
    ...
    async run(o) {
        ...
        console.log(global.gojodev);

        send({
            body: ``,
            attachment: global.gojodev.splice(0, 1)
        });
    }
